...
if (isset($_GET['date_from'])) {
    ...
    // Default: Jan 1 of the year that contains the latest posted entry.
    // This keeps date_from = date_to even when the cloud has only historical data.
    try {
        $_dr = $pdo->query(
        ...
        if ($_dr && $_dr['max_d']) {
            ...
            $date_from = $max_year . '-01-01';   // Jan 1 of latest-data year
            $date_to   = $_dr['max_d'];
        } else {
...
// Divisions and blocks are pre-filtered to the user's company & business unit when assigned.
...
try {
    ...
    // Divisions: scope to user's business unit if assigned, else scope to company if assigned
    if ($sess_bu_id) {
    ...
    // Blocks: scope to user's business unit if assigned, else scope to company if assigned
    if ($sess_bu_id) {
    ...
    // Activities: always full list (not org-scoped)
...
<div class="container-fluid mt-4">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="block_id" class="form-select" id="blockFilter">
                            ...
                            <?php foreach ($blocks as $block): ?>
                                <option value="<?= $block['block_id'] ?>" data-division="<?= $block['division_id'] ?>" <?= $block_id == $block['block_id'] ? 'selected' : '' ?>>
    ...
    <?php
    // Note: Adjust these aliases if your table uses different column names
    // Current assumption: journal_entries uses 'id' as primary key
    // If your table uses 'entry_id', change the alias below
    $entry_id_column = "je.id"; // Change to "je.entry_id" if needed
    ...
    // that the P&L queries do not have. Used by profit_loss and profit_loss_detail.
...
// Export functions
// Active filter dates injected from PHP so export always uses the correct range
// even when the dates were auto-detected (not present in the URL).
...
function exportToExcel(reportType) {
    ...
    // Always override with the PHP-resolved dates (handles auto-detected defaults)
    params.set('date_from', _activeDateFrom);
    params.set('date_to',   _activeDateTo);
    params.set('export', 'excel');

    // Pass print-info so the Excel header matches the PDF/print header
    ...
    // Grab translated labels from the first available print-area data-i18n block
...
/**
 * Opens a clean print-only window containing only the Profit & Loss table.
 * Strips navbar, sidebar, KPI cards, card wrapper, filters — everything except
 * the company header + period + bare <table>.
 */
function printProfitLoss() {
    ...
    // Hide GL drill-down column in print (no-print cells already hidden by CSS;
    // but in the cloned DOM we remove them explicitly so colspan stays clean)
    ...
    const company     = document.querySelector('meta[name="x-company"]')?.content || 'AI based AGROBUSINESS SOLUTION';
    const businessUnit= document.querySelector('meta[name="x-business-unit"]')?.content || '';
    const printBy  = document.querySelector('meta[name="x-username"]')?.content || '';
    const now      = new Date();
    const pad      = n => String(n).padStart(2, '0');
    const printDT  = `${pad(now.getDate())}/${pad(now.getMonth()+1)}/${now.getFullYear()} ${pad(now.getHours())}:${pad(now.getMinutes())}`;
...
<style>
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .rpt-bu      { font-size: 9.5pt; font-weight: 600; color: #166c82; margin-top: 1px; }
...
<div class="rpt-header">
  ...
  ${businessUnit ? '<div class="rpt-bu">' + escHtml(businessUnit) + '</div>' : ''}
...
/**
 * Same as printProfitLoss() but for the Detail P&L report (#pld-print-area).
 */
function printDetailPL() {
    ...
    // Clone the print area and strip interactive elements
...
<style>
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .rpt-bu      { font-size: 10pt; font-weight: 600; color: #166c82; margin-top: 1px; }
 ...
 * Period header uses "As at DD/MM/YYYY" (point-in-time).
 */
...
// -- Shared helper: open a clean print window with given area, title, subtitle --
...
function printGeneralLedger() {
    ...
    const periodL      = i18n.period   || 'Period';
    const printByL     = i18n.print_by || 'Print by';
    const dateTimeL    = i18n.datetime || 'Date/Time';
    const company      = document.querySelector('meta[name="x-company"]')?.content    || 'AI based AGROBUSINESS SOLUTION';
    const businessUnit = document.querySelector('meta[name="x-business-unit"]')?.content || '';
    ...
    // Clone print area and strip interactive / screen-only elements
    const areaClone = area.cloneNode(true);
    areaClone.querySelectorAll('.no-print').forEach(el => el.remove());
    areaClone.querySelectorAll('a').forEach(a => {
        const span = document.createElement('span');
...
<style>
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .rpt-printinfo { text-align: right; font-size: 7.5pt; color: #555; line-height: 1.6; margin-bottom: 6px; }
  .rpt-header { text-align: center; border-bottom: 2px solid #166c82; padding-bottom: 10px; margin-bottom: 16px; }
  .rpt-company { font-size: 11pt; font-weight: 700; color: #166c82; }
  .rpt-bu      { font-size: 9pt; font-weight: 600; color: #166c82; margin-top: 1px; }
  .rpt-title   { font-size: 13pt; font-weight: 700; margin: 4px 0 2px; }
  .rpt-period  { font-size: 8.5pt; color: #555; }
  .card { border: 1px solid #ccc; border-radius: 4px; page-break-inside: avoid; margin-bottom: 8px; }
  .card-header { padding: 4px 8px; font-size: 8.5pt; font-weight: 700; background-color: #166c82 !important; color: #fff !important; border-radius: 3px 3px 0 0; display:flex; justify-content:space-between; }
  .card-body { padding: 0; }
  .table-responsive { overflow: visible; }
  ...
  .text-danger  { color: #1a1a1a !important; }
  .text-success { color: #1a1a1a !important; }
  .fw-semibold, .fw-bold, strong { font-weight: 700; }
  .fst-italic { font-style: italic; }
  .badge { display: inline-block; padding: 1px 4px; border-radius: 3px; font-size: 7pt; }
  code { font-family: monospace; font-size: 85%; }
...
<div class="rpt-header">
  ...
  ${businessUnit ? '<div class="rpt-bu">' + escHtml(businessUnit) + '</div>' : ''}
  <div class="rpt-title">${escHtml(title)}</div>
  <div class="rpt-period">${escHtml(periodL)}: <strong>${escHtml(dateFrom)}</strong> &rarr; <strong>${escHtml(dateTo)}</strong></div>
</div>
${areaClone.innerHTML}
</body>
</html>`;

    // Use a hidden iframe so browser pop-up blockers don't interfere
