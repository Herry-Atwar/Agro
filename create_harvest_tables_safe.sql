-- ============================================
-- SAFE Fix for Missing harvest_realizations Table
-- This version creates tables WITHOUT foreign key constraints
...
-- Harvest Plans Table (NO FOREIGN KEYS)
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Harvest planning without FK constraints';

-- Harvest Realizations Table (THE CRITICAL TABLE - NO FOREIGN KEYS)
CREATE TABLE harvest_realizations (
    ...
    INDEX idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Actual harvest records - MAIN TABLE';

-- Harvest Productivity Table (NO FOREIGN KEYS)
CREATE TABLE harvest_productivity (
    ...
    INDEX idx_harvest_date (harvest_date)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Harvester performance tracking';

-- Harvest Quality Control Table (NO FOREIGN KEYS)
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Quality inspection records';

-- Insert sample data (only if block_id 1 and 2 exist)
...
INSERT IGNORE INTO harvest_realizations (harvest_number, harvest_plan_id, block_id, harvest_date,
...
('HV-2024-0003', 2, 2, '2024-01-10', 4600, 230, 100, 20.0, 'Round 1', 7,
 'Jane Supervisor', 'Grade B', 'Ripe', 'Sunny', 'Truck-01', 'Main Mill', 'In Transit', 'admin');

-- Verify tables were created
SELECT 'SUCCESS: All harvest tables created successfully!' AS Status;
SELECT COUNT(*) as harvest_realizations_count FROM harvest_realizations;
