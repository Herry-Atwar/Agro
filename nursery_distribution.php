...
if (is_post()) {
    ...
    if ($action == 'add') {
        try {
            ...
            // Insert distribution record using positional parameters
            $stmt = $db->prepare("
                INSERT INTO nursery_distributions
                (nursery_stock_id, block_id, distribution_date, quantity_distributed, planting_date,
                 receiver_name, vehicle_number, status, notes, created_at, updated_at)
                VALUES
                (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
            ...
            $update_stmt = $db->prepare("
                ...
                SET quantity_ready = quantity_ready - ?,
                    status = CASE
                        WHEN quantity_ready - ? <= 0 THEN 'Distributed'
                        ELSE status
                    END
                WHERE id = ?
            ");
            $update_stmt->execute([$quantity_distributed, $quantity_distributed, $stock_id]);
...
if (get('action') == 'edit' && get('id')) {
    $stmt = $db->prepare("
        ...
        JOIN   nursery_stocks ns ON nd.nursery_stock_id = ns.id
        WHERE  nd.id = ?
    ");
    $stmt->execute([get('id')]);
