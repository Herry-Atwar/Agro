...
if (is_post()) {
    ...
    if ($action == 'add') {
        try {
            $stmt = $db->prepare("
                INSERT INTO nursery_maintenances
                (nursery_stock_id, activity_date, activity_type, description,
    ...
    elseif ($action == 'edit') {
        try {
            $stmt = $db->prepare("
                ...
                SET nursery_stock_id = ?, activity_date = ?, activity_type = ?,
                    description = ?, quantity_affected = ?,
                    materials_used = ?, labor_hours = ?, cost = ?,
                    performed_by = ?, notes = ?, updated_at = NOW()
                WHERE id = ?
            ");
            $stmt->execute([
                post('stock_id'),
    ...
    elseif ($action == 'delete') {
        try {
            $stmt = $db->prepare("DELETE FROM nursery_maintenances WHERE id = ?");
            $stmt->execute([post('maintenance_id')]);
...
if (get('action') == 'edit' && get('id')) {
    $stmt = $db->prepare("SELECT *, id as maintenance_id, nursery_stock_id as stock_id FROM nursery_maintenances WHERE id = ?");
    $stmt->execute([get('id')]);
...
$params = [];
if ($search) {
    $sql .= " AND (ns.batch_number LIKE ? OR nm.description LIKE ? OR nm.performed_by LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
if ($stock_filter) {
    $sql .= " AND nm.nursery_stock_id = ?";
    $params[] = $stock_filter;
}
if ($activity_filter) {
    $sql .= " AND nm.activity_type = ?";
    $params[] = $activity_filter;
}
if ($date_from) {
    $sql .= " AND nm.activity_date >= ?";
    $params[] = $date_from;
}
if ($date_to) {
    $sql .= " AND nm.activity_date <= ?";
            ...
            <table class="table table-hover table-sm">
                <thead>
                    <tr>
                        ...
                        <th>Activity Type</th>
                        <th>Description</th>
                        <th class="text-end">Qty Affected</th>
                ...
                <tbody>
                    <?php if (empty($activities)): ?>
                        <tr>
                            <td colspan="11" class="text-center text-muted py-4">No production activities found</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($activities as $activity): ?>
                            <tr>
                                ...
                                <td class="text-end"><?php echo $activity['quantity_affected'] ? number_format($activity['quantity_affected'], 0) : '-'; ?></td>
                                <td><?php echo $activity['materials_used'] ? htmlspecialchars($activity['materials_used']) : '-'; ?></td>
                                <td class="text-end"><?php echo $activity['labor_hours'] ? number_format($activity['labor_hours'], 1) : '-'; ?></td>
                                <td class="text-end">Rp <?php echo number_format($activity['cost'] ?? 0, 0); ?></td>
                                <td><?php echo $activity['performed_by'] ? htmlspecialchars($activity['performed_by']) : '-'; ?></td>
                                <td class="text-nowrap">
                                    <div class="btn-group" role="group">
                                        <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#viewModal<?php echo $activity['maintenance_id']; ?>" title="View">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        ...
                                        <button type="button" class="btn btn-sm btn-danger" title="Delete"
                                            onclick="if(confirm('Delete this activity?')) { document.getElementById('deleteForm<?php echo $activity['maintenance_id']; ?>').submit(); }">
                                            <i class="bi bi-trash"></i>
                                        </button>
                                    </div>
                                    <form id="deleteForm<?php echo $activity['maintenance_id']; ?>" method="POST" action="nursery_production.php" style="display:none;">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <table class="table table-sm">
                            <tr><th width="40%">Qty Affected:</th><td><?php echo $activity['quantity_affected'] ? format_number($activity['quantity_affected'], 0) : '-'; ?></td></tr>
                            <tr><th>Labor Hours:</th><td><?php echo $activity['labor_hours'] ? format_number($activity['labor_hours'], 1) : '-'; ?></td></tr>
                            <tr><th>Cost:</th><td>Rp <?php echo format_number($activity['cost'] ?? 0, 0); ?></td></tr>
                            <tr><th>Performed By:</th><td><?php echo $activity['performed_by'] ? htmlspecialchars($activity['performed_by']) : '-'; ?></td></tr>
                        </table>
                    </div>
                </div>
                <?php if ($activity['description']): ?>
                <div class="mt-2"><h6>Description:</h6><p><?php echo nl2br(htmlspecialchars($activity['description'] ?? '')); ?></p></div>
                <?php endif; ?>
                <?php if ($activity['materials_used']): ?>
                <div class="mt-2"><h6>Materials Used:</h6><p><?php echo htmlspecialchars($activity['materials_used']); ?></p></div>
                <?php endif; ?>
                <?php if ($activity['notes']): ?>
                <div class="mt-2"><h6>Notes:</h6><p><?php echo nl2br(htmlspecialchars($activity['notes'] ?? '')); ?></p></div>
                <?php endif; ?>
            </div>
